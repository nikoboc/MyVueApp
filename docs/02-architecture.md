...
└── MonthlyView.vue                  ルート "/monthly/:month?"
    └── PunchForm.vue                (日付も選ばせる。editable-date="true")
```

`PunchForm` は 2 か所から使う。日ごとのカードから使う場合、どの日の打刻かはカード自体で決まって
いるため日付欄を出さない。月次集計から使う場合は、**記録が 1 件も無い日**を選べる必要があるため
日付欄を出す。丸ごと打刻を忘れた日は日ごとのカードが存在せず、打刻画面からは追加できないためで
ある。表示中の月と違う日付が入力された場合は、追加した記録が見えるようにその月へ移動する。
