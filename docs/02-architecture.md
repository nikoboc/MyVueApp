...
初期値に入る。ただしこの形式は出勤・休憩・退勤を 1 組ずつしか持てないため、中抜けで出退勤が 2 回ある
日や休憩を複数回取った日は表しきれない。`toDayEntry` はその場合 `isLossy` を立て、ダイアログは
「一部しか表せない」ことを明示して打刻画面での修正を案内する。黙って一部だけ拾うと、保存した時点で
残りが失われる。実際、中抜けの日（実働 8 時間）をこの形式で保存すると 9 時間になる。
